...
class LookupOfflineDBProvider {
  ...
  Future<bool> checkProdutoLookUp(endpoint, data, tipos, empresaID) async {
    ...
    final res = await db.query(TABLE_ProdutoLookUp, where: "tipos = ? and endpoint and lookupNome = ?",
  ...
  Future<Map<String, dynamic>> getProdutoLookUp(endpoint, empresaId, tipos) async {
                      ...
                      final db = await database;
                 List<Map> res = await db.query(TABLE_ProdutoLookUp, where: "endpoint = ? and tipos = ?", whereArgs: [endpoint, tipos]);
    List<Offline> listaOffline = res.isNotEmpty ? res.map((c) => Offline.fromJson(c)).toList() : [];
  ...
  createProdutoLookUp(empresaId, tipo, search, situacao, response) async {
  
    var newProdutoLookUp = {
      "codigo": empresaId.toString(),
      "descricao": tipo.toString(),
      "descricaoResumida": json.encode(response),
      "marca": DateTime.now().toString(),
      "saldoEstoque": tipo.toString(),
      "valorVenda": tipo.toString(),
      "unidadeMedida": tipo.toString(),
      "locacaoBens": tipo.toString(),
      "lookupNome": tipo.toString(),
      "empresaId": tipo.toString(),
      "search": tipo.toString(),
      "tipos": tipo.toString(),
      "situacoes": tipo.toString(),
