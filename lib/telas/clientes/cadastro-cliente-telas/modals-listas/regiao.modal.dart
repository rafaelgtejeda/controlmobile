...
class RegiaoModal extends StatefulWidget {

  RegiaoModal({Key key}) : super (key: key);

  @override
  _RegiaoModalState createState() => _RegiaoModalState();
}

class _RegiaoModalState extends State<RegiaoModal> {
  ...
  List<RegiaoLookUp> _lista = new List<RegiaoLookUp>();
  String _pesquisa = '';
  Timer _debounce;
  ...
  Future<dynamic> _fazRequest() async {
    ...
    if (!_infinite.infiniteScrollCompleto) {
      ...
      dynamic requestLista = await ClienteService().regiao.listaRegiao(
        skip: _infinite.skipCount,
        search: _pesquisa
      );
      List<RegiaoLookUp> listaRequest = new List<RegiaoLookUp>();
      requestLista.forEach((data) {
        listaRequest.add(RegiaoLookUp.fromJson(data));
      });
    ...
    return LocalizacaoWidget(
      child: StreamBuilder(
        builder: (context, snapshot) {
          return Scaffold(
            appBar: AppBar(
              title: Text(_locate.locale[TraducaoStringsConstante.SelecaoRegiao]),
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: _locate.locale[TraducaoStringsConstante.BuscarRegiao],
  ...
  Widget _item(BuildContext context, int index, RegiaoLookUp regiao) {
    if (index >= _lista.length) {
      return null;
    }
    return InkWell(
      onTap: () {
        Navigator.pop(context, regiao);
      },
      child: Padding(
