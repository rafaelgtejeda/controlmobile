...
class TabelaPrecoModal extends StatefulWidget {

  TabelaPrecoModal({Key key}) : super (key: key);

  @override
  _TabelaPrecoModalState createState() => _TabelaPrecoModalState();
}

class _TabelaPrecoModalState extends State<TabelaPrecoModal> {
  ...
  List<TabelaPrecoLookUp> _lista = new List<TabelaPrecoLookUp>();
  String _pesquisa = '';
  Timer _debounce;
  ...
  Future<dynamic> _fazRequest() async {
    ...
    if (!_infinite.infiniteScrollCompleto) {
      ...
      dynamic requestLista = await ClienteService().tabelaPreco.listaTabelaPreco(
        skip: _infinite.skipCount,
        search: _pesquisa
      );
      List<TabelaPrecoLookUp> listaRequest = new List<TabelaPrecoLookUp>();
      requestLista.forEach((data) {
        listaRequest.add(TabelaPrecoLookUp.fromJson(data));
      });
    ...
    return LocalizacaoWidget(
      child: StreamBuilder(
        builder: (context, snapshot) {
          return Scaffold(
            appBar: AppBar(
              title: Text(_locate.locale[TraducaoStringsConstante.SelecaoTabelaPreco]),
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: _locate.locale[TraducaoStringsConstante.BuscarTabelaPreco],
  ...
  Widget _item(BuildContext context, int index, TabelaPrecoLookUp tabelaPreco) {
    if (index >= _lista.length) {
      return null;
    }
    return InkWell(
      onTap: () {
        Navigator.pop(context, tabelaPreco);
      },
      child: Padding(
