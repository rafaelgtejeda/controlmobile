...
class GrupoContatoModal extends StatefulWidget {

  GrupoContatoModal({Key key}) : super (key: key);

  @override
  _GrupoContatoModalState createState() => _GrupoContatoModalState();
}

class _GrupoContatoModalState extends State<GrupoContatoModal> {
  ...
  List<GrupoContatoLookUp> _lista = new List<GrupoContatoLookUp>();
  String _pesquisa = '';
  Timer _debounce;
  ...
  Future<dynamic> _fazRequest() async {
    ...
    if (!_infinite.infiniteScrollCompleto) {
      ...
      dynamic requestLista = await ClienteService().grupoContato.listaGrupoContato(
        skip: _infinite.skipCount,
        search: _pesquisa
      );
      List<GrupoContatoLookUp> listaRequest = new List<GrupoContatoLookUp>();
      requestLista.forEach((data) {
        listaRequest.add(GrupoContatoLookUp.fromJson(data));
      });
    ...
    return LocalizacaoWidget(
      child: StreamBuilder(
        builder: (context, snapshot) {
          return Scaffold(
            appBar: AppBar(
              title: Text(_locate.locale[TraducaoStringsConstante.SelecaoGrupoContato]),
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: _locate.locale[TraducaoStringsConstante.BuscarGrupoContato],
  ...
  Widget _item(BuildContext context, int index, GrupoContatoLookUp grupoContato) {
    if (index >= _lista.length) {
      return null;
    }
    return InkWell(
      onTap: () {
        Navigator.pop(context, grupoContato);
      },
      child: Padding(
