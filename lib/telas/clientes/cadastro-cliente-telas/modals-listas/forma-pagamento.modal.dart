...
class FormaPagamentoModal extends StatefulWidget {

  final int parceiroId;
  final int tipoRetorno;

  FormaPagamentoModal({Key key, this.parceiroId, this.tipoRetorno}) : super (key: key);

  @override
  _FormaPagamentoModalState createState() => _FormaPagamentoModalState();
}

class _FormaPagamentoModalState extends State<FormaPagamentoModal> {
  ...
  List<FormaPagamentoLookup> _lista = new List<FormaPagamentoLookup>();
  String _pesquisa = '';
  Timer _debounce;
  ...
  Future<dynamic> _fazRequest() async {
    ...
    if (!_infinite.infiniteScrollCompleto) {
      ...
      dynamic requestLista = await ClienteService().limiteCredito.formaPagamento.listaFormas(
        skip: _infinite.skipCount,
        search: _pesquisa,
        tipoRetorno: widget.tipoRetorno,
        parceiroId: widget.parceiroId
      );
      List<FormaPagamentoLookup> listaRequest = new List<FormaPagamentoLookup>();
      requestLista.forEach((data) {
        listaRequest.add(FormaPagamentoLookup.fromJson(data));
      });
    ...
    return LocalizacaoWidget(
      child: StreamBuilder(
        builder: (context, snapshot) {
          return Scaffold(
            appBar: AppBar(
              title: Text(_locate.locale[TraducaoStringsConstante.SelecaoFormaRecebimento]),
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: _locate.locale[TraducaoStringsConstante.BuscarFormaRecebimento],
  ...
  Widget _item(BuildContext context, int index, FormaPagamentoLookup formaRecebimento) {
    if (index >= _lista.length) {
      return null;
    }
    return InkWell(
      onTap: () {
        Navigator.pop(context, formaRecebimento);
      },
      child: Padding(
            ...
            RichText(
              text: TextSpan(
                ...
                children: <InlineSpan> [
                  ...
                  TextSpan(
                    text: formaRecebimento.descricao,
                  ),
                ]
              )
            ),
