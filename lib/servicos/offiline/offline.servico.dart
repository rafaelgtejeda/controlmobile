...
class OfflineService {
  ...
  Future<void> sincronizacaoDownload(BuildContext context) async {
    ...
    if (confirmacao && await _requestUtil.verificaOnline()) {
      ...
      EmpresaEUsuario empEUser = EmpresaEUsuario.fromJson(empresasEUsuario);
  ...
  Future<bool> _sincronizaEmpresaAcessos({int empresaId, List<Empresa> listaEmpresas}) async {
    bool retorno = false;
  ...
  Future<void> _sincronizaClientes({int empresaId, List<Empresa> listaEmpresas}) async {
    bool retorno = false;
    ...
    if (resultado.length != 0) {
      ...
      retorno = await ClienteLookupProvider().insertClientesBatch(resultado, listaEmpresas);
    }
    return retorno;
  }
